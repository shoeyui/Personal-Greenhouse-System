    ...
    while(1){
        ...
        */
        ...
        if (a=='1'){
            ...
            delay(1000);
            str_pos(1, 2);
            send_str("1 - Sensor Info");
            delay(100);
            str_pos(2, 2);

            if(fanState==0){
            send_str("2 - Turn Fan On");
            }

            if(fanState==1){
            send_str("2 - Turn Fan Off");
            }
            delay(100);
            str_pos(3, 2);

            if(panelState==0){
             send_str("4 - Turn LEDs On");
                }

             if(panelState==1){
                  send_str("4 - Turn LEDs Off");
              }

             str_pos(4, 2);
             if (heatState==0){
                 send_str("5 - Turn Heat On");
             }
             if (heatState==1){
                 send_str("5 - Turn Heat Off");
             }
